...
for i, m in enumerate(m_values):
    ...
    rho_C = 0.5 * Cp**2 + 0.5 * m**2 * C**2
    p_C   = 0.5 * Cp**2 - 0.5 * m**2 * C**2
    w_C = p_C / rho_C
    
    plt.plot(a, w_C, label=fr"$w_\mathcal{{C}}(a), \ m={m}$", color=colors[i])

plt.axhline(-1, color="red", linestyle="--", label=r"$w_\mathcal{C}=-1$")
plt.axhline(0, color="black", linestyle=":", alpha=0.6)

plt.legend(loc="upper right", fontsize=14, frameon=True)

param_text = (r"$V(\mathcal{C}(a)) = \frac{1}{2} m^2 (\mathcal{C}(a))^2$" "\n"
