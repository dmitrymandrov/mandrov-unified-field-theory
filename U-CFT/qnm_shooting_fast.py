...
def main():
    ...
    plt.xlabel(r'$\mathrm{Re}(\omega)$', fontsize=12)
    plt.ylabel(r'$\mathrm{Im}(\omega)$', fontsize=12)
    plt.title(r'$\log_{10}|A_{\mathrm{in}}(\omega)|$ for U–CFT metric', fontsize=14)
    plt.tick_params(axis='both', which='major', labelsize=10)
    cbar = plt.colorbar(label=r'$\log_{10}|A_{\mathrm{in}}|$')
    cbar.ax.tick_params(labelsize=10)
    cbar.set_label(r'$\log_{10}|A_{\mathrm{in}}|$', fontsize=12)
    plt.tight_layout()
    plt.savefig("qnm_ucft.png", dpi=300)
